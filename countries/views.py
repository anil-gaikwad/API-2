...
from rest_framework import status , Http404

from countries.models import WCountries
...
def countries_list(request):
    if request.method == 'GET':
        countries = WCountries.objects.all()
...
def countries_detail(request, pk):
    try:
        countries = WCountries.objects.get(pk=pk)
    except WCountries.DoesNotExist:
